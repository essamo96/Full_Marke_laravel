...
<body>
  <div class="email-container">
    ...
    <div class="content">
      <p>مرحباً <strong>{{ $student->full_name_ar ?? $student->full_name_en }}</strong>،</p>

      <div class="highlight-box">
        <p>{{ $suspended ? '⚠️ تم إغلاق حسابك في بوابة الطالب' : '✅ تم إعادة تفعيل حسابك' }}</p>
      </div>

      @if($suspended)
        <p>{{ __('app.mail_account_suspended_body', ['amount' => number_format($totalDue, 2)]) }}</p>
      @else
        <p>{{ __('app.mail_account_reactivated_body') }}</p>
      @endif
    </div>
