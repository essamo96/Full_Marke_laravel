...
@elseif ($x == 2)
    <a href="{{ route($active_menu . '.view') }}"
        class="btn btn-outline  btn-outline-dashed btn-outline-info btn-active-light-info btn-sm">{{ $name }}</a>
@elseif ($x == 3)
    <a
        class="btn btn-outline  btn-outline-dashed btn-outline-warning  btn-active-light-info btn-sm">{{ $name }}</a>
@elseif ($x == 4)
    @if ($image)
        <button type="button"
        ...
        <div class="modal fade" id="imageModal-{{ $id }}" tabindex="-1"
            aria-labelledby="imageModalLabel-{{ $id }}" aria-hidden="true">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <div class="ms-auto">
                            ...
                            <a class="bi bi-download text-info" title="{{ \App\Helpers\translate('download_image') }}"
                                href="{{ asset('storage/' . $image) }}" download></a>

                            <button type="button" class="btn-close ms-2" data-bs-dismiss="modal"
                                aria-label="{{ \App\Helpers\translate('cancel') }}"></button>
                        </div>
                    </div>
